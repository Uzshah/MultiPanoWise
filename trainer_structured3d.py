...
from utils.metrics_s3d import compute_depth_metrics, Evaluator, compute_shading_metrics, compute_alb_metrics, compute_semantic_metrics
from utils.metrics_s3d import compute_normal_metrics, mIoU
...
class Trainer_s3d:
    def __init__(self, settings):
        ...
        train_dataset = Structured3D(self.settings.data_path, "", num_classes = 41,
        ...
        val_dataset = Structured3D(self.settings.data_path, "", num_classes = 41,
                                   height=settings.input_size[0], width=settings.input_size[1],
    ...
    def process_batch(self, inputs, is_training = True):
        ...
        # ALbedo & noraml loss
        losses['joint_loss'] = ssim(inputs["rgb"], outputs["pred_shading"] * outputs["pred_albedo"]).mean()
        ...
        if is_training:
            losses['loss'] = self.settings.alpha * torch.max(
                    torch.max(losses["depth_loss"], losses["normal_loss"]),
                    torch.max(losses['semantic_loss'], torch.max(losses["albedo_loss"], losses["shading_loss"])), 
                ) + (1 - self.settings.alpha) * (
                    losses["depth_loss"] + losses["normal_loss"] +
                    losses['semantic_loss'] + losses["albedo_loss"] +
                    losses["shading_loss"]
                )
        else:
            losses['loss'] = 0
    ...
    def log(self, mode, inputs, outputs, losses, batch_idx=0):
        ...
        for j in range(min(2, self.settings.batch_size)):  # write a maxmimum of four images
            ...
            if self.settings.target == "all" or self.settings.target == "shading":
                writer.add_image("gt_shading/{}-{}".format(j, batch_idx), 
                                 inputs["shading"][j].data/inputs["shading"][j].max(),self.step)
                writer.add_image("pred_shading/{}-{}".format(j, batch_idx), 
                                 outputs["pred_shading"][j].data/outputs["pred_shading"][j].max(),self.step)
                
            if self.settings.target == "all" or self.settings.target == "albedo":
                
                writer.add_image("gt_albedo/{}-{}".format(j, batch_idx), 
                                 inputs["albedo"][j].data/inputs["albedo"][j].max(),self.step)
                writer.add_image("pred_albedo/{}-{}".format(j, batch_idx), 
                                 outputs["pred_albedo"][j].data/ outputs["pred_albedo"][j].max(),self.step)
                
            if self.settings.target == "all" or self.settings.target == "normal":
                writer.add_image("gt_normal/{}-{}".format(j, batch_idx), 
                                 inputs["normal"][j].data/inputs["normal"][j].max(),self.step)
                writer.add_image("pred_normal/{}-{}".format(j, batch_idx), 
                                 outputs["pred_normal"][j].data/outputs["pred_normal"][j].max() ,self.step)
