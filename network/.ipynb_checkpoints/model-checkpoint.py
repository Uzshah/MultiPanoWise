...
class Panoformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
        ...
        self.context_sh = Context(1)
        # self.context_de = Context(1)
        self.context_alb = Context(3)
        ...
        self.shading = OutputProj(in_channel=2 * embed_dim, out_channel=1, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
        self.semantic = OutputProj(in_channel=2 * embed_dim, out_channel=41, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
        self.albedo = OutputProj(in_channel=2 * embed_dim, out_channel=3, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
        self.normal = OutputProj(in_channel=2 * embed_dim, out_channel=3, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
    ...
    def forward(self, x):
        ...
        # Output Projection
        # normal = self.normal(deconv3)
        depth = self.depth(deconv0)
        semantic = self.context_se(self.semantic(deconv0), z)
        # albedo = self.context_alb(self.albedo(deconv3), z)
        # shading = self.context_sh(self.shading(deconv3), z)
        outputs = {}
        # outputs["pred_normal"] = normal
        outputs["pred_depth"] = depth
        outputs["pred_semantic"] = semantic
        # outputs["pred_albedo"] = albedo
        # outputs["pred_shading"] = shading
        return outputs
